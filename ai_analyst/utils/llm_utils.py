...
@contextmanager
def gemma3_session(model_path: str = MODEL_PATH, load_in_4bit: bool = LOAD_4BIT):
    """
    ...
    _           = AutoProcessor.from_pretrained(model_path, use_fast=True)  # not used but required
...
def analyst_inference(messages, *, max_new_tokens: int = MAX_TOK_GEN) -> str:
    """One assistant turn given `messages` (Open‑AI style list of dicts)."""
    with gemma3_session() as (model, tokenizer):
